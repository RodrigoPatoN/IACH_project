...
def main(model_path, dataset_path, output_path):
    #load model
    svm = SVMModel.from_file(model_path)
    ...
    preds = svm.predict(dataset['SentimentText'].to_numpy()).flatten()
...
if __name__ == "__main__":
    if len(sys.argv) > 1: #running from main.py, with args
        ...
        main('./temp/temp_svm.keras', './temp/temp_predict_data.csv', './temp/temp_predictions.csv')
